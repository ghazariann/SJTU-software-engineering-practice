...
void Board::move(int from, int to, bool log) {
  ...
  if (from < 0 || from > 2 || to < 0 || to > 2)
    return;
  if (rods[from]->empty() || rods[to]->full()) //  bug fix: check empty() first
    return;
...
bool Board::win() { return rods[1]->size() == num_disk; }

void solve(
    int n, int src, int buf, int dest,
    Queue<pair<int, int>> &solution) { // bug fix: get &solution not solution
  if (n >= 1) {
