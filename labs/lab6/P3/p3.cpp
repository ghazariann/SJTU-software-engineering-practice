...
int Solution::dfs(vector<vector<char>> &board, int index,
  ...
  int avail_count = to_be_solved[index][2];
  int ret;

  for (int k = 1; k <= 9; k++) {
    if (!row_avail[x][k - 1] && !col_avail[y][k - 1] &&
        !grid_avail[to_grid_index(x, y)][k - 1]) { // fix 1: row_avail[x] not y
      row_avail[x][k - 1] = 1;
      col_avail[y][k - 1] = 1;
      ...
      board[x][y] = '0';
      // fix 2: reset all row_avail, col_avail, grid_avail 0
      row_avail[x][k - 1] = 0;
      col_avail[y][k - 1] = 0;
...
void Solution::solveSudoku(vector<vector<char>> &board) {
  ...
  sort(
      to_be_solved.begin(), to_be_solved.end(),
